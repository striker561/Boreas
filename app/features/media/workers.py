...
from app.core.worker_heartbeats import QUEUE_MEDIA, write_worker_heartbeat
...
async def warm_media_worker(ctx: dict[str, Any]) -> None:
    ...
    await write_worker_heartbeat(ctx, queue=QUEUE_MEDIA)
...
async def ingest_media_job(ctx: dict[str, Any], job_id: str) -> None:
    ...
    await write_worker_heartbeat(ctx, queue=QUEUE_MEDIA, job_id=job_id)
    try:
